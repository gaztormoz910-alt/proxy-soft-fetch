...
# ------------------- ограничение одновременного разбора (голодание UI)

def test_throttled_parse_returns_the_same_result():
    content = "8.8.8.8:8080\n1.1.1.1:3128\n"
    assert sorted(ProxyUtils.parse_proxies_throttled(content)) == \
           sorted(ProxyUtils.parse_proxies(content))


def test_parse_concurrency_is_capped():
    """Разбор идёт под GIL. Когда все 50 качающих потоков разбирают
    одновременно, главный поток Tk — один из 51 претендента на GIL, и окно
    получает от Windows статус «Не отвечает»."""
    import threading
    import time

    peak = {"n": 0, "cur": 0}
    lock = threading.Lock()
    real = ProxyUtils.parse_proxies
    ...
    ProxyUtils.parse_proxies = staticmethod(slow_parse)
    try:
        threads = [threading.Thread(target=ProxyUtils.parse_proxies_throttled,
                                    args=("8.8.8.8:8080",)) for _ in range(30)]
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=10)
    finally:
        ProxyUtils.parse_proxies = staticmethod(real)

    assert peak["n"] <= ProxyUtils.PARSE_CONCURRENCY, (
        f"одновременно разбирало {peak['n']} потоков при лимите "
        f"{ProxyUtils.PARSE_CONCURRENCY}")
    assert peak["n"] > 1, "ограничитель не должен сводить разбор к одному потоку"


def test_the_gate_is_released_when_parsing_raises():
    """Иначе первое же исключение навсегда съест слот ограничителя."""
    real = ProxyUtils.parse_proxies

    def boom(content):
        raise ValueError("битый источник")

    ProxyUtils.parse_proxies = staticmethod(boom)
    try:
        for _ in range(ProxyUtils.PARSE_CONCURRENCY + 2):
            with pytest.raises(ValueError):
                ProxyUtils.parse_proxies_throttled("что угодно")
    finally:
        ProxyUtils.parse_proxies = staticmethod(real)

    # слоты должны быть свободны
    assert ProxyUtils.parse_proxies_throttled("8.8.8.8:8080") == ["8.8.8.8:8080"]
